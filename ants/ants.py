...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if self.can_contain(place.ant): #adding container ant
                self.store_ant(place.ant)
                place.ant = self
            elif place.ant.can_contain(self): #adding other ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            if place.bees:
                if self.lower_bound <= bee_distance <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= amount:
            ref_dame += self.damage 
        for bee in self.place.bees[:]:
            bee.reduce_health(ref_dame)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew: #the ant is chewing
            self.turns_to_chew -= 1
        elif not self.turns_to_chew: #the ant are available to chew
            if self.place.bees:
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while ant_place:
            if ant_place.ant:
                ant_place.ant.double()
                if ant_place.ant.is_container and ant_place.ant.ant_contained:
                    ant_place.ant.ant_contained.double()
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def target_action(gamestate):
            if self.turns_remain: #The bee still being slowed
                self.turns_remain -= 1
                if gamestate.time % 2 == 0: #The bee only take action at even time
                    return Bee.action(target, gamestate)
                else: #The bee does nothing
                    return 
            else:
                return Bee.action(target, gamestate)

        if target is not None: #from thrower ant
            self.turns_remain = self.slow_turns
